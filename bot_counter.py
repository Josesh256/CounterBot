...
# Diccionario para almacenar el conteo de enlaces de X por usuario y fecha
user_x_links = {}
...
async def count_x_links(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if x_links:
        ...
        if today not in user_x_links[chat_id]:
            user_x_links[chat_id][today] = {}

        if user_id not in user_x_links[chat_id][today]:
            user_x_links[chat_id][today][user_id] = 0
        
        user_x_links[chat_id][today][user_id] += len(x_links)
        logger.info(f"Usuario {update.message.from_user.full_name} ha compartido {len(x_links)} enlaces de X en {today}.")

# Función para mostrar el conteo de enlaces de X por usuario para hoy
async def show_x_links_count(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.message.chat.id
    today = datetime.now().strftime('%Y-%m-%d')  # Obtener la fecha de hoy

    if chat_id in user_x_links and today in user_x_links[chat_id]:
        counts = user_x_links[chat_id][today]
        report_lines = []
        for user_id, count in counts.items():
            user = await context.bot.get_chat(user_id)
            report_lines.append(f"{user.full_name}: {count} enlaces de X")
        report = "\n".join(report_lines)
        await update.message.reply_text(f"Conteo de enlaces de X para hoy ({today}) en este chat:\n{report}")
    else:
        await update.message.reply_text(f"No se han compartido enlaces de X en este chat hoy ({today}).")
